...
file write table "\documentclass[border=0pt]{standalone}" _n
file write table "\usepackage{graphicx}" _n
file write table "\usepackage{tabularx}" _n
file write table "\usepackage{caption}" _n
file write table "\usepackage{changepage}" _n
...
file write table "\begin{minipage}{1.0\textwidth}" _n
file write table _n

file write table "\begin{table}[h!]" _n
file write table "    \caption{Summary Statistics and T-test}" _n
file write table "    \begin{tabularx}{\textwidth}{|X|X|X|X|X|X|X|X|}" _n
file write table "        \hline" _n
file write table "        & \multicolumn{3}{c|}{\textbf{Treatment}} & \multicolumn{3}{c|}{\textbf{Control}} & \\" _n
file write table "        \hline" _n
file write table "         \makecell{Variable} & \makecell{Mean} & \makecell{Std. Dev.} & \makecell{N} & \makecell{Mean} & \makecell{Std. Dev.} & \makecell{N} & \makecell{P-value} \\" _n
file write table "        \hline" _n


* Write data rows here dynamically using Stata variables
